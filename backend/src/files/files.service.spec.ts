...
describe('FilesService', () => {
  ...
  describe('uploadFile', () => {
    ...
    it('should handle file cleanup error gracefully', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'Could not cleanup temporary file:',
        expect.any(Error),
      );
  ...
  describe('findFilesByUser', () => {
    it('should fail when user does not exist', async () => {
      ...
      await expect(service.findFilesByUser('nonexistent-id')).rejects.toThrow(
    ...
    it('should fail when user has no documents', async () => {
      ...
      await expect(service.findFilesByUser(mockUser.id)).rejects.toThrow(
        `No documents found for user ${mockUser.email}`,
      );
    });
  ...
  describe('findOne', () => {
    ...
    it('should fail with null id', async () => {
      ...
      await expect(service.findOne(null as any)).rejects.toThrow(
        'Document with id null not found',
      );
    });
    ...
    it('should fail with undefined id', async () => {
      ...
      await expect(service.findOne(undefined as any)).rejects.toThrow(
        'Document with id undefined not found',
      );
    });
  });
  ...
  describe('getFileHistoryByUserId', () => {
    it('should fail when user in history does not exist', async () => {
      ...
      await expect(service.getFileHistoryByUserId(mockUser.id)).rejects.toThrow(
  ...
  describe('downloadFile', () => {
    it('should fail when file does not exist on disk', async () => {
      ...
      await expect(
        service.downloadFile(mockFile.id, mockResponse),
    ...
    it('should fail when document does not exist', async () => {
      ...
      await expect(
        service.downloadFile('nonexistent-id', mockResponse),
  ...
  describe('streamFile', () => {
    it('should fail when document has no filePath', async () => {
      ...
      await expect(service.streamFile(mockFile.id)).rejects.toThrow(
        'File path not found',
      );
    });
  ...
  describe('update', () => {
    it('should fail when user does not exist', async () => {
      ...
      await expect(
        service.update(mockFile.id, undefined, 'nonexistent@example.com', {
          name: 'new name',
        }),
      ).rejects.toThrow('User with email nonexistent@example.com not found');
    });
    ...
    it('should fail when document status is not allowed', async () => {
      const approvedFile = {
        ...mockFile,
        currentStatusId: 'approved-status-id',
      };
      (mockFileRepo.findOne as jest.Mock).mockResolvedValue(approvedFile);
      (mockUserService.findOneByEmail as jest.Mock).mockResolvedValue(mockUser);

      await expect(
        service.update(mockFile.id, undefined, mockUser.email, {
          name: 'new name',
        }),
      ).rejects.toThrow(
    ...
    it('should fail when new file path does not exist', async () => {
      const rejectedFile = {
        ...mockFile,
        currentStatusId: DocumentStatus.REJECTED,
      };
      ...
      await expect(
        service.update(mockFile.id, newFile, mockUser.email, {
          name: 'new name',
        }),
    ...
    it('should handle old file deletion error gracefully', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Could not delete old file'),
        expect.any(Error),
      );
      expect(result).toBeDefined();
  ...
  describe('verifyFileIntegrity', () => {
    it('should return false when document has no fileBuffer', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'Document does not have a file buffer to verify',
      );
    ...
    it('should return false when document has no fileHash', () => {
      ...
      expect(result).toBe(false);
      expect(consoleSpy).toHaveBeenCalledWith(
        'Document does not have a file hash to compare against',
      );
    ...
    it('should throw InternalServerErrorException when crypto fails', () => {
      ...
      expect(() =>
        service.verifyFileIntegrity(fileWithBufferAndHash as any),
  ...
  describe('getStatusTypes', () => {
    it('should throw InternalServerErrorException when repository fails', async () => {
      (mockStatusRepo.find as jest.Mock).mockRejectedValue(
        new Error('Database error'),
      );
      const consoleSpy = jest.spyOn(console, 'error').mockImplementation();

      await expect(service.getStatusTypes()).rejects.toThrow(
        'Could not fetch document status types',
      );

      expect(consoleSpy).toHaveBeenCalledWith(
        'Error fetching document status types:',
        expect.any(Error),
      );
  ...
  describe('findDocumentStatus', () => {
    it('should throw InternalServerErrorException when repository throws', async () => {
      (mockStatusRepo.findOne as jest.Mock).mockRejectedValue(
        new Error('Database connection lost'),
      );
      const consoleSpy = jest.spyOn(console, 'error').mockImplementation();

      await expect(service['findDocumentStatus']('any-id')).rejects.toThrow(
        'Error finding document status',
      );

      expect(consoleSpy).toHaveBeenCalledWith(
        'Error finding document status with id any-id:',
        expect.any(Error),
      );
  ...
  describe('getFilesHistory', () => {
    it('should fail when user in history does not exist', async () => {
      ...
      await expect(service.getFilesHistory()).rejects.toThrow(
        'User with id nonexistent-user-id not found',
      );
    });
  });
  ...
  describe('Edge Cases and Data Integrity', () => {
    it('should handle extremely large file sizes', async () => {
      ...
      const mockReadFileSync = jest
        .spyOn(require('fs'), 'readFileSync')
        .mockReturnValue(largeFile.buffer);
      ...
      const result = await service.uploadFile(
        largeFile,
        { name: 'large-file.pdf', description: 'test' },
        mockUser.email,
      );
      expect(result).toBeDefined();
    ...
    it('should handle special characters in file names', async () => {
      ...
      const result = await service.uploadFile(
        specialCharFile,
        { name: 'test-файл', description: 'test' },
        mockUser.email,
      );
      expect(result).toBeDefined();
    ...
    it('should handle concurrent duplicate file uploads', async () => {
      ...
      const mockReadFileSync = jest
        .spyOn(require('fs'), 'readFileSync')
        .mockReturnValue(Buffer.from('identical content'));
      const mockUnlinkSync = jest
        .spyOn(require('fs'), 'unlinkSync')
        .mockImplementation(() => {});
